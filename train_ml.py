...
parser.add_argument( '--accpath', default='backbone', type=str)
parser.add_argument( '--pklpath', default='backbone', type=str)
#Device options
...
parser.add_argument('--runs', default='runs-888', type=str)
parser.add_argument('-n', '--num', default='1', type=int)
parser.add_argument('--mode', default='SAG', type=str)
parser.add_argument('--dataset', default='airound', type=str,
                    help='dataset')
parser.add_argument('--dim', default='128', type=int)
parser.add_argument('--t_c', default='3', type=float)
parser.add_argument('--t_s', default='2', type=float)
...
def main():
    ...
    # create model
    pklpath_o=arch+"-"+args.mode+"-"+str(args.at)+"-"+str(args.fusion)+"-"+str(args.dim)+"-"+str(args.dp)+"-"+str(args.traing_ratio)+"-"+str(args.num)
    if args.arch=="resnet":      
        t_model=resnet(num_class,args.archname,args.mode,args.at,args.fusion,args.dim,args.dp)
        s_model=resnet(num_class,args.archname,args.mode,args.at,args.fusion,args.dim,args.dp)
        s_model.load_state_dict(torch.load(os.path.join('E:\\results\\airound\\xr', pklpath_o+".pkl")))
        t_model.load_state_dict(torch.load(os.path.join('E:\\results\\airound\\xr', pklpath_o+".pkl")))
    ...
    size=sum(p.numel() for p in s_model.parameters())/1000000.0
    ...
    pklpath=arch+"-"+args.mode+"-"+str(args.at)+"-"+str(args.fusion)+"-"+str(args.dim)+"-"+str(args.dp)+"-"+str(args.traing_ratio)+"-"+str(args.t_s)+"-"+str(args.t_c)+"-"+str(args.num)
    ...
    if os.path.exists(os.path.join('results',args.accpath, args.runs+".csv")):
        ...
        csv_writer = csv.writer(f)
        csv_writer.writerow(['arch','acc','dataset','trainratio','op','lr','schedule','bz','epoch','num','size','run_time'])


    # Train and val
    acc=[]
    t=0
    test_loss, test_acc, _, _, _ = test(val_loader, s_model, criterion, 1, use_cuda)
    for epoch in range(start_epoch, 500): 
        t=epoch
        if optimizer.param_groups[0]['lr']<0.001:
            break
        print('\nEpoch: [%d | %d] LR1: %f ' % (epoch + 1, args.epochs, optimizer.param_groups[0]['lr']))
        train_loss, train_acc = train(train_loader, t_model, s_model,criterion, optimizer, epoch, use_cuda,criterion2) 
        if optimizer.param_groups[0]['lr']<0.01:
                update_moving_average(ema_updater, t_model, s_model)
        logger.append([epoch + 1,optimizer.param_groups[0]['lr'], train_loss,  train_acc])          
        lr_scheduler.step(train_loss)
    test_loss, test_acc,test1,test2,test3 = test(val_loader, s_model, criterion, 1, use_cuda)
    acc.append(test_acc)
    endtime=time.time()
    ...
    csv_writer.writerow([arch+"-"+args.mode, '%.4f'%test_acc,
                        ...
                        str(t),str(args.num),'%.2f'%size,str(runtime),str( args.at),str( args.fusion),str( args.dim),str( args.dp),'%.4f'%test1,'%.4f'%test2,'%.4f'%test3,str(args.t_s),str(args.t_c)])
    f.close()
    check_path(os.path.join('E:\\results\\airound', args.pklpath))
    filepath = os.path.join('E:\\results\\airound', args.pklpath, pklpath+".pkl")
    torch.save(s_model.state_dict(), filepath)


def train(train_loader, t_model, s_model,criterion, optimizer, epoch, use_cuda,criterion2):
    ...
    for batch_idx, (s_sample,a_sample,g_sample,targets,path) in enumerate(train_loader):
        ...
        with torch.no_grad():
            t_outputs = t_model(s_sample,a_sample,g_sample)
        s_outputs = s_model(s_sample,a_sample,g_sample)
        rand=np.random.randint(0,3,(bs))
        s_a_sample,s_g_sample,s_s_sample=a_sample.detach().clone(),g_sample.detach().clone(),s_sample.detach().clone()
        s_a_sample[np.where(rand==0),:,:,:]=0
        s_g_sample[np.where(rand==1),:,:,:]=0
        s_s_sample[np.where(rand==2),:,:,:]=0
        s_s_outputs = s_model(s_s_sample,s_a_sample,s_g_sample)

        T_s = args.t_s
        T_c=args.t_c
        loss_cls = criterion(s_outputs, targets)
        loss_cis = criterion2(F.log_softmax(s_outputs/T_c,dim=1), F.softmax(t_outputs/T_c,dim=1))
        loss_s_cis= criterion2(F.log_softmax(s_s_outputs/T_s,dim=1), F.softmax(t_outputs/T_s,dim=1))
        loss=(loss_cls+loss_cis+loss_s_cis)/3
